...
function eval_(exp, env) {
  ...
  if (selfEval(exp)) return eval(exp); // JS eval for native type
    ...
    if (expanded) {
      ...
      return apply(op, operands(exp), env);
    }
  }
...
addPrimitive('prit', ['elements*'], 
  function(operands, env) {
    ...
    for (var m = 1, opr; opr = operands[m]; m++) { str += eval_(opr, env).toString(); };
    pr(str); return;
  }),
...
addPrimitive('.', ['list', 'index'], opEval(
  function(operands, env) {
...
['+', '-', '*', '/'].forEach(function(op) {
  addPrimitive(op, ['loperand', 'roperand'], opEval(typeDispatch(op)));
});

addPrimitive('join', ['list', 'separator'], opEval(typeDispatch('join')));
addPrimitive('length', ['stuff'], opEval(typeDispatch('length')));
addPrimitive('push', ['array', 'element'], opEval(typeDispatch('push')));
...
addPrimitive('split', ['string', 'separator', 'maxSplit'], opEval(typeDispatch('split')));
