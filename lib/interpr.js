...
function leftCurry(l, params) {
  var curry = lambdaCurry(l) || [];
  var lparams = lambdaParams(l);
  var numset = 0;
  for (var m = 0; m < lparams.length; m++) 
    if (!curry[m] && curry[m] != 0) curry[m] = params[numset++];
...
var primitives = {
  ...
  'lcurry': opEval(function(operands, env) {
    var fn = operands.first();
    if (!lambda(fn)) throw "The first parameter of leftCurry should be a function."
    return makeCurriedLambda(fn, leftCurry(fn, operands.tail()));
  }),
  'rcurry': opEval(function(operands, env) {
    var fn = operands.first();
    if (!lambda(fn)) throw "The first parameter of rightCurry should be a function."
    return makeCurriedLambda(fn, rightCurry(fn, operands.tail()));
  }),
  'ncurry': opEval(function(operands, env) {
    var fn = operands.first();
    if (!lambda(fn)) throw "The first parameter of rightCurry should be a function."
    var idx = operands[1];
    if (typeof idx != "number") throw "Second parameter of ncurry must be a number."
    return makeCurriedLambda(fn, nCurry(fn, idx, operands.last()));
  }),
  '=': function(operands, env) {
    var name = operands[0];
