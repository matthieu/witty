function patternMatch(pattern, datum, frame) {
  ...
  else if (pattern.length > 1 && datum.length > 1 && (pattern instanceof Array) && (datum instanceof Array)) 
...
function escapeEval(exps, env) {
  ...
  for (var m = 0; m < exps.length; m++) {
    if (copy[m] instanceof Array) copy[m] = escapeEval(copy[m], env);
      ...
      print(copy[m] + "->" + variableValue(copy[m].substring(1), env));
      copy[m] = variableValue(copy[m].substring(1), env);
    }
  }
  return copy;
}
