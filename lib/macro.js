function patternMatch(pattern, datum, frame) {
  print("1:: " + (pattern) + " : " + (datum));
  print("2:: " + (pattern instanceof Array) + " : " + (datum instanceof Array));
  if (pattern instanceof Array) print("3:: " + (pattern.sntx) + " : " + (datum.sntx) + " : " + macroVar(pattern[0]));
  ...
  else if ((pattern instanceof Array) && (datum instanceof Array) && pattern.sntx != datum.sntx) return 'fail';
  else if ((pattern instanceof Array) && (datum instanceof Array) && pattern.length > datum.length) return 'fail';
  else if ((pattern instanceof Array) && (datum instanceof Array) && pattern.length == 0 && datum.length == 0) return frame;
  else if ((pattern instanceof Array) && (datum instanceof Array) && pattern.length == 1 && datum.length > 1 && macroVar(pattern[0])) {
    return extendIfConsistent(pattern[0], datum, frame);
  }
  else if (pattern.length > 0 && datum.length > 0 && (pattern instanceof Array) && (datum instanceof Array)) 
...
function expandMacros(exps, env) {
  ...
  for (var m = 0, el; el = exps[m]; m++) {
    ...
    if (val && macro(val)) {
      if (macroApplics[val[3]]) macroApplics[val[3]].unshift([m, val]); 
...
function rewriteWithMacro(exps, env, macroApplics, index, mac) {
  ...
  print("offs: " + offset + " / frame: " + JSON.stringify(frame));
  if (frame) {
    expansion = eval_(mac[2], extendEnv(frame, env));
    print("splicing " + mac[1].length + " from exps " + exps);
...
function matchAt(index, pattern, exps) {
  ...
  print("idx " + index + " exps " + exps[index] + " " + JSON.stringify(extract));
...
function escapeEval(exps, env) {
  ...
  print("rewrote: " + JSON.stringify(copy));
  return copy;
}
